...
def song_data_prep(songs_file, interactions_file, export_dir):
    ...
    with open(os.path.join(export_dir,'user_song_tuple.txt'), 'wb') as handle:
...
def find_subnetwork(network_type, dir, factor=0.1):
    ...
    if 'dense' in network_type:
        if 'top' in network_type:
            ...
            start = 0
        song_nodes_holder = song_degree[start:int(len(
    ...
    print(f'The {network_type} subnetwork has {len(nodes)} nodes: {len(song_nodes)} songs, {len(user_nodes)} users, {len(person_nodes)} persons.')
    ...
    for i in tqdm(nodes_set):  # (node1, node2) and (node2, node1) both exist
        connect_1 = set(song_user[i]).intersection(nodes_set) # song-user
        for j in connect_1:
            edges_type1.append((i, j))

        connect_2 = set(song_person[i]).intersection(nodes_set) # song-person
        for j in connect_2:
            edges_type2.append((i, j))

        connect_3 = set(user_song[i]).intersection(nodes_set) # user-song
        for j in connect_3:
            edges_type3.append((i, j))

        connect_4 = set(person_song[i]).intersection(nodes_set) # person-song
        for j in connect_4:
...
def ix_mapping(network_type, import_dir, export_dir, mapping_export_dir):
    ...
    type_to_ix = {'person': consts.PERSON_TYPE, 'user': consts.USER_TYPE, 'song': consts.ITEM_TYPE,
                  pad_token: consts.PAD_TYPE}
    relation_to_ix = {'song_person': consts.ITEM_PERSON_REL, 'person_song': consts.PERSON_ITEM_REL,
                      'user_song': consts.USER_ITEM_REL, 'song_user': consts.ITEM_USER_REL, '#UNK_RELATION': consts.UNK_REL,
                      '#END_RELATION': consts.END_REL, pad_token: consts.PAD_REL}

    # entity vocab set is combination of songs, users, and persons
    if network_type == 'full':
        song_data_prefix = import_dir
    else:
        song_data_prefix = import_dir + network_type + '_'
    with open(song_data_prefix + consts.ITEM_USER_DICT, 'rb') as handle:
        song_user = pickle.load(handle)
    with open(song_data_prefix + consts.ITEM_PERSON_DICT, 'rb') as handle:
        song_person = pickle.load(handle)
    with open(song_data_prefix + consts.USER_ITEM_DICT, 'rb') as handle:
        user_song = pickle.load(handle)
    with open(song_data_prefix + consts.PERSON_ITEM_DICT, 'rb') as handle:
        person_song = pickle.load(handle)
    ...
    # Id-ix mappings
    entity_to_ix = {(song, consts.ITEM_TYPE): ix for ix, song in enumerate(songs)}
    ...
    # Update the KG
    song_user_ix = convert_to_ids(entity_to_ix=entity_to_ix, rel_dict=song_user, start_type=consts.ITEM_TYPE, end_type=consts.USER_TYPE)
    user_song_ix = convert_to_ids(entity_to_ix, user_song, consts.USER_TYPE, consts.ITEM_TYPE)
    song_person_ix = convert_to_ids(entity_to_ix, song_person, consts.ITEM_TYPE, consts.PERSON_TYPE)
    person_song_ix = convert_to_ids(entity_to_ix, person_song, consts.PERSON_TYPE, consts.ITEM_TYPE)
...
def main():
    ...
    # DATA_LOC = os.path.join(r'/data2/Leigh', 'Data', consts.DATASET_DOMAIN) # Game-01
    DATA_LOC = os.path.join(r'/home/hananb/Leigh', 'Data', consts.DATASET_DOMAIN) # Game-05
    ...
    # create_directory(os.getcwd()+'/'+consts.SONG_DATA_DIR)
    # define current locations

    song_file_loc = os.path.join(DATA_LOC, consts.ITEM_DATASET_DIR, args.songs_file)
    interaction_file_loc = os.path.join(DATA_LOC, consts.ITEM_DATASET_DIR, args.interactions_file)
    song_data_prep(songs_file=song_file_loc, interactions_file=interaction_file_loc,
